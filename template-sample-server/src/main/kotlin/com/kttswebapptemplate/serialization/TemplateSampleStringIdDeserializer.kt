...
class TemplateSampleStringIdDeserializer<T : TemplateSampleStringId>(
    val templateSampleStringIdClass: KClass<T>
) : StdDeserializer<T>(templateSampleStringIdClass.java) {

    companion object {
        fun <T : TemplateSampleStringId> deserialize(
            templateSampleStringIdClass: KClass<T>,
            value: String
        ): T =
            TemplateSampleSerializationPrefixUtils.removePrefix(templateSampleStringIdClass, value)
                .let { templateSampleStringIdClass.constructors.first().call(it) }
    }
